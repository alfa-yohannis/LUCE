...
```
Also replace all the ip addresses `127.0.0.1:8545` in files `/home/ryan/LUCE/luce_vm/luce_django/django_binary_db_field/utils/web3_scripts.py` and `/home/ryan/LUCE/luce_vm/luce_django/luce/utils/web3_scripts.py` with `192.168.0.4:8545`, which are the ip address of the host/local machine where the `ganache-cli` runs.
...
- Use the commands below to check if the network connections between the nodes are well established.
  ...
  ping docker exec [node_name] ping [node_ip_adress | node_name] # to check if a node is reachable from the current node
